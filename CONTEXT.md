...
**Team-Match**:
One team's perspective on a **Fixture** — the team-level fact row. A Fixture has exactly two Team-Match rows. The unit a team **Rolling Window** iterates over. Every row records **which source produced it**. League team data is split by **recency**, not by provider: the **historical** record is football-data.co.uk's, while **recently-played** Fixtures are written from ESPN within minutes of the final whistle, and football-data.co.uk reclaims the row when it eventually publishes (see `docs/adr/0015`). Provenance is not bookkeeping — "has this source published?" is asked **per source**, so a row's mere existence cannot answer it.
_Avoid_: assuming a league Team-Match came from football-data.co.uk.
...
A Metric's value can also depend on **which source recorded it**, and the sources do not always agree on what to count. football-data.co.uk scores a two-booking dismissal as one red card and **no** yellows; FBref counts both bookings — three yellows against one for the same match (Leeds v Crystal Palace, 15 Mar 2026). Left unreconciled this changes a Metric's meaning mid-**Rolling Window**, flipping ~1.7% of *yellows 3+* verdicts on source alone. Any source writing Team-Match rows must therefore be **reconciled to football-data.co.uk's convention** before its rows are trusted in a window (`docs/adr/0015`).

Separately, a source can simply be **wrong**. Sampling the 25 worst football-data.co.uk/FBref disagreements against ESPN as arbiter confirmed **27 erroneous values in football-data.co.uk** and **6 in FBref** — mostly a ±10 transcription slip, twice a whole fixture's figures swapped with another's. No source is assumed correct: a correction requires **two independent sources agreeing against the third**.
_Avoid_: stat (informal), market; treating two sources' figures for the same Metric as interchangeable.
...
The presence of a fact row is **not** proof the owed source published it. A league **Team-Match** may have been written from ESPN while football-data.co.uk is still silent (`docs/adr/0015`), so the question is asked against the row's recorded **source**, never its existence — otherwise a fallback write would silently mute the alarm for the source that actually failed, absorbing the outage this definition exists to expose.
...
**Promotion Play-offs**:
The end-of-season knockout deciding the last promotion place (Championship/League One/League Two): teams finishing 3rd–6th contest two-legged semi-finals (4 fixtures) then a single neutral-venue final. Modelled as its **own competition** (one per division — "Championship Play-offs", "League One Play-offs", "League Two Play-offs" — **Competition Type** `club_cup`), never as part of the regular season — a play-off leg shares the same home/away orientation as a league meeting, so keeping it under `club_league` collided on the Fixture natural key and contaminated league form (see `docs/adr/0004`). A "last N **league** games" window therefore excludes them by scope. football-data.co.uk does not cover play-offs, so **both** the player rows and the team rows come from FBref's cached match pages — the team rows via the same zero-network pass the cups use. Ingested in the season they are played: ESPN's `season.slug` (`promotion-semifinals` / `promotion-final`) routes the finished fixture to the play-off competition, which is what makes the round visible to match-day's pending probe at all (`docs/adr/0017`).
...
The set of players currently *registered* to a club, as listed on that club's **ESPN roster** — *membership*, not appearance history. A new signing who has not played yet is in the Squad; a player loaned out is not (he appears at his loan club). Refreshed daily into the `squads` table by a tier-1 unattended job (see `docs/adr/0013`), which supersedes ADR 0006's decision to defer a roster source and its FBref-squad-page plan. Held only for clubs with an `espn_id` — the four English tiers; a non-league cup opponent or a foreign club has no Squad and falls back to **Recent squad**. Distinct from a **Player-Match**/**Appearance** (what a player *did*): the Squad decides *who* is shown, Appearances supply *the numbers*.
...
The set of players whose most-recent **Appearance** in covered data was for this club — "the squad as it last took the field." Derived at query time from each player's date-ordered **Player-Match** rows: the `team_id` of his `MAX(date)` appearance (the stored `current_team_id` is *not* trusted — it is last-written, not guaranteed chronological). No roster source. Accurate *during* a season; lags the summer transfer window — a sold player lingers in his old club's Recent squad until he debuts elsewhere in covered data (the accepted **"ghost"**), and a new signing appears only once he debuts. Always surfaced labelled as appearance-based, never implied to be the registered **Squad**. Was the v1 membership rule for **Squad form** (ADR 0006); since `docs/adr/0013` it serves two narrower jobs: the **fallback** for a club with no ESPN roster, and the **safety net** inside Squad form's membership (see below). Its weakness is what prompted the change — a player whose next club is outside covered data never leaves, so Wolves still listed Diego Costa and Patrick Cutrone, 25 of 44 members stale and 10 of them multi-year.
_Avoid_: calling it the "current squad" without the appearance-based caveat.

**Squad form** (not lineup):
Recent Summary Metrics for each player in a team's **Squad** — membership from the ESPN roster, the form numbers from that player's **Appearances** at this club. Membership is the Squad **plus anyone who has appeared for the club in the last 30 days**. That union is a safety net, not a widening: a player who turned out last week is self-evidently still there, whatever spelling the roster used, so an unreconciled name degrades the panel to *slightly stale* rather than *silently missing a real player*. It is deliberately short — a season-long union would keep a January departure until August, which is the very staleness this replaced. A Squad member with no **Appearances** we hold (a new signing, a third-choice keeper) is shown with no figure rather than omitted, sorted last: "in the squad, nothing known" is different from "not in the squad". One reusable panel, shown on two surfaces: the **Fixture view** (both teams side by side, its primary home) and the **Team hub** (one team, a jump-off into its players). Never an auto-predicted XI. Filtering to confirmed starters is a manual user step once the official lineup is released. The form numbers were unchanged by the move from **Recent squad** to **Squad** — only *who* is listed changed.
