...
from ingestion import coverage, players, run_backfill, upcoming
...
def run_matchday(
    ...
    for comp in plan:
        ...
        results[comp] = code
        # Team rows are built by run_backfill itself — it owns the cache the
        # pass reads, so every caller gets them, not just this one.
        _sweep_orphans(log)
